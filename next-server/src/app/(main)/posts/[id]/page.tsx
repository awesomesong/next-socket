...
import { FormEvent, useState, use, useMemo } from "react";
...
const Post = ({ params }: { params: Promise<Props> }) => {
  ...
  // ✅ DOMPurify sanitize를 useMemo로 최적화
  ...
  // ✅ 댓글 텍스트 sanitize 함수
  const sanitizeCommentText = useMemo(() => {
    return (text: string) => {
            ...
            <div className="flex flex-col gap-4 w-full break-all">
              ...
              <div className="flex flex-col gap-4">
                ...
                {post?.postComments.map((postComment) => (
                  <div key={postComment.id} className="flex flex-col gap-1">
                    <div className="flex flex-row">
                      <div className="flex flex-row gap-2 items-center flex-wrap">
                        <AvatarWithName
                          image={postComment.writer?.image ?? ''}
                          name={postComment.writer?.name ?? '(알 수 없음)'}
                        />
                        <span>
                          {dayjs(new Date(Number(postComment.createdAt))).fromNow()}
                        </span>
                      </div>
                      {session?.user.email === postComment.writer?.email && (
                        ...
                        >
                          <Tooltip showArrow={true} content="삭제" size="lg">
                    ...
                    <pre
                      dangerouslySetInnerHTML={{ __html: sanitizeCommentText(postComment.text || "") }}
