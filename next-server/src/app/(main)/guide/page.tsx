...
const features = [
  ...
  },
  {
    icon: '📷',
    title: 'AI 이미지 분석 입력',
    desc: '향수 병 이미지를 업로드하면 OpenAI GPT-4o Vision으로 브랜드·향수명·향 노트 등을 분석해 향수 등록 폼을 자동으로 채워 줍니다. 비어 있거나 잘못된 항목만 직접 수정하면 되고, 신규 향수 등록이 훨씬 빨라집니다.',
  },
  {
    icon: '⭐',
    title: '리뷰 시스템',
    desc: '향수 상세 페이지에서 나만의 감상을 리뷰로 남기고 수정·삭제할 수 있습니다. 무한 스크롤로 다른 사용자의 리뷰도 이어서 볼 수 있으며, 새 리뷰는 실시간으로 목록에 반영됩니다.',
  },
  {
    icon: '📋',
    title: '공지사항',
    desc: '서비스 소개, 이용 방법, 업데이트 내역을 공지로 안내합니다. WYSIWYG 에디터(Quill)로 이미지·서식을 포함한 글을 작성할 수 있고, 댓글로 의견을 남길 수 있습니다. 업로드한 이미지는 Cloudinary CDN으로 제공되며, DOMPurify로 악성 스크립트를 차단해 안전하게 표시됩니다.',
  },
  {
    icon: '💬',
    title: '실시간 채팅',
    desc: 'Socket.IO 기반 1:1·그룹 채팅으로 다른 사용자와 실시간 대화합니다. 상대방이 오프라인 상태여도 메시지는 서버에 보관(최대 24시간)되어 재접속 시 자동으로 전달됩니다.',
  },
  {
    icon: '🤖',
    title: 'AI 어시스턴트',
    desc: 'GPT-4 기반 AI에게 향수 추천이나 궁금한 점을 물어볼 수 있습니다. 답변은 타이핑되는 것처럼 실시간 스트리밍(SSE)으로 출력되어 기다림 없이 자연스럽게 대화할 수 있습니다.',
  },
  {
    icon: '✨',
    title: '3D 배경',
    desc: 'Three.js를 사용하여 메인 화면에 인터랙티브 3D 은하수 배경을 구현했습니다. 스크롤하면 은하수 속으로 빨려 들어가는 듯한 부드러운 확대 효과를 경험할 수 있습니다.',
  },
  {
    icon: '🌙',
    title: '다크 모드 및 반응형',
    desc: '라이트/다크 테마 전환 기능을 지원하며, 데스크탑과 태블릿, 모바일 디바이스에 맞춘 반응형 웹 디자인을 적용했습니다.',
  },
  {
    icon: '🔐',
    title: '간편 로그인 & 데모 계정',
    desc: 'Google·카카오 OAuth와 이메일/비밀번호(Credentials) 로그인을 지원하며, 별도 회원가입 없이 바로 체험 가능한 데모 계정을 제공합니다. 모든 기능은 NextAuth 기반 JWT 세션으로 보호되어 안전하게 이용할 수 있습니다.',
  },
];
...
const fragranceDetailSteps: FragranceGuideStep[] = [
  ...
  {
    step: '02',
    title: '비로그인 상태 — 리뷰 입력칸 미표시',
    desc: '로그인하지 않은 상태에서는 향수 상세 정보(이미지·브랜드·이름·설명·노트)만 확인할 수 있습니다. 우측 상단에는 갤러리로 돌아가는 목록 버튼만 표시되며, 리뷰 입력칸은 나타나지 않습니다.',
  ...
  {
    step: '03',
    title: '로그인 상태 — 리뷰 입력칸 & 수정·삭제 버튼 활성화',
    desc: '로그인하면 리뷰 섹션 상단에 "리뷰를 입력해주세요." 입력칸이 활성화됩니다. 해당 향수를 직접 등록한 사용자라면 우측 상단에 목록·수정·삭제 버튼도 함께 표시됩니다.',
  ...
  },
  {
    step: '04',
    title: '리뷰 작성 & 제출',
    desc: '입력칸에 감상을 작성하고 확인 버튼을 누르면 리뷰가 등록됩니다. 제출한 내용은 실시간으로 다른 사용자 화면에도 즉시 반영됩니다.',
...
const noticeWriteGuideSteps: FragranceGuideStep[] = [
  ...
  },
  {
    ...
    desc: '로그인한 사용자에게는 우측 상단에 글쓰기 버튼이 표시됩니다. 버튼을 클릭하면 공지사항 작성 페이지로 이동합니다.',
  ...
  },
  {
    step: '03',
    title: '글쓰기 페이지',
    desc: '글쓰기 버튼을 클릭하면 공지사항 작성 페이지로 이동합니다. 상단의 제목 입력란에 제목을 입력하고, 아래 에디터(Quill)에서 본문을 작성합니다. 에디터 툴바에서 굵게·기울임·밑줄·취소선 등의 텍스트 서식을 적용하거나, 이미지 아이콘을 클릭해 본문에 이미지를 삽입할 수 있습니다. 작성이 완료되면 등록 버튼을 클릭해 공지를 제출하거나, 취소 버튼을 눌러 목록으로 돌아갈 수 있습니다.',
  ...
  },
  {
    ...
    desc: '등록 버튼을 클릭하면 작성한 공지사항이 저장되고 해당 공지의 상세 페이지로 이동합니다. 상세 페이지에서는 제목, 작성자, 조회 수, 댓글 수와 함께 본문 내용을 확인할 수 있습니다. 작성자 본인에게는 우측 상단에 수정·삭제 버튼이 표시되며, 하단에는 댓글을 작성할 수 있는 입력란이 제공됩니다.',
...
const chatConversationGuideSteps: FragranceGuideStep[] = [
  {
    step: '01',
    title: '대화방 탭 진입',
    desc: '채팅 메뉴에서 대화방 탭을 선택하면 내가 참여한 대화 목록이 표시됩니다. 데스크탑에서는 좌측 사이드바 상단의 "대화방" 제목 우측에 더보기(⋮) 버튼이 위치하며, 참여 중인 대화방이 없으면 목록에 "대화방이 없습니다."라고 표시됩니다. 우측 메인 화면에는 "대화방이나 멤버를 선택하면 채팅이 시작됩니다."라는 안내와 함께, 하단의 "향수 AI 어시스턴트와 채팅" 버튼을 클릭해 AI 채팅방으로 바로 이동할 수 있습니다. 모바일에서는 화면 상단의 "대화방" 제목 우측에 더보기(⋮) 버튼이 있으며, 참여 중인 대화방이 없을 경우 화면 중앙에 "대화방이 없습니다."라는 안내가 표시됩니다.',
  ...
  },
  {
    step: '02',
    title: '대화방 더보기 메뉴(⋮)',
    desc: '"대화방" 제목 우측에 위치한 더보기(⋮) 버튼을 클릭하면 다음 세 가지 메뉴가 나타납니다. ① 단체 채팅: 여러 멤버가 함께 참여하는 그룹 채팅방을 만들 수 있습니다. ② AI 채팅: 향수 AI 어시스턴트와의 채팅방으로 바로 이동합니다. ③ 다크 모드 / 라이트 모드: 현재 테마를 즉시 전환합니다.',
  ...
  },
  {
    ...
    desc: '임시 대화방이 생성되고 해당 채팅창이 열립니다. 상단에는 방 이름과 참여 인원 수가 보이고, 하단 입력창에서 첫 메시지를 보내면 그때 서버에 대화방이 생성됩니다. 생성이 완료되면 대화방 목록에 새 방이 나타나고, 보낸 메시지가 채팅창에 표시됩니다.',
...
const chatDetailSteps: FragranceGuideStep[] = [
  {
    ...
    desc: '대화방 채팅창 우측 상단의 ⋮ 버튼을 누르면 채팅방 서랍이 열립니다. 서랍에서는 현재 참여 중인 멤버 목록을 확인하고 대화방을 나갈 수 있습니다. 내가 보낸 메시지 아래에는 "읽음: [닉네임]" 형태로 읽음 상태가 표시됩니다. 상대방이 메시지를 읽으면 읽음 표시가 자동으로 업데이트됩니다.',
  ...
  },
  {
    ...
    desc: '⋮ 버튼을 누르면 오른쪽에서 채팅방 서랍이 열립니다. 서랍 상단에는 대화방 이름과 총 참여 인원 수가 표시되고, 그 아래에는 참여 멤버의 프로필·이름·이메일이 나열됩니다. 본인 계정에는 "나" 배지가, 현재 온라인 상태인 멤버는 프로필 우측 상단에 초록색 점이 표시됩니다. 서랍 하단의 "대화방 나가기" 버튼을 클릭하면 해당 대화방에서 나갈 수 있습니다.',
  ...
  },
  {
    step: '05',
    title: '나가기 완료',
    desc: '대화방에서 나가면 "대화방이 삭제되었습니다." 토스트 메시지가 표시되고, 대화방 목록으로 자동으로 돌아옵니다. 나간 대화방은 목록에서 사라지며, 해당 대화의 모든 기록은 복구할 수 없습니다.',
...
const chatAiGuideSteps: FragranceGuideStep[] = [
  ...
  },
  {
    ...
    desc: '질문을 입력해 전송하면 AI가 답변 생성을 시작합니다. 응답이 준비되는 동안에는 다음 메시지 전송이 제한되며, 답변이 완료되면 다시 이어서 질문을 보낼 수 있습니다.',
...
const mainGalleryGuide: MainGalleryGuide = {
  ...
  desc: [
    <span key="1">메인 페이지는 상단의 <strong>3D 히어로 섹션</strong>과 그 아래 <strong>시그니처 향수 갤러리</strong>로 구성되어 있으며, 아래로 스크롤하면 갤러리로 자연스럽게 이어집니다.</span>,
    <span key="2">갤러리에서는 상단의 <strong>브랜드 필터</strong>로 원하는 브랜드를 선택하면 해당 브랜드의 향수만 볼 수 있습니다. 필터는 스크롤 시 상단에 고정되고, 위로 스크롤하면 숨겨졌다가 아래로 스크롤하면 다시 나타납니다. 모바일에서는 펼치기 버튼으로 전체 브랜드 목록을 확인할 수 있습니다.</span>,
    <span key="3">갤러리 목록은 무한 스크롤로 불러오며, 카드를 클릭하면 해당 향수의 상세 페이지로 이동합니다. 로그인한 경우 갤러리 우측 상단(데스크탑) 또는 제목 아래(모바일)의 <strong>향수 정보 추가</strong> 버튼으로 새 향수를 등록할 수 있습니다.</span>,
  ],
