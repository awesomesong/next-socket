...
import { replaceCommentById, removeCommentById, blogsCommentsKey, incrementBlogDetailCommentsCount } from '@/src/app/lib/blogsCache';
import type { BlogCommentsDataProps } from '@/src/app/types/blog';
...
const Comments = ({ blogId, user } : CommentsProps) => {
    ...
    const isAllCommentsLoaded = (): boolean => {
        const cached = queryClient.getQueryData(blogsCommentsKey(blogId)) as BlogCommentsDataProps | undefined;
        if (!(cached && Array.isArray(cached.pages))) return false;
        const totalLoaded = cached.pages.reduce((acc: number, page) => {
            const p = page.find((item) => 'comments' in item) as { comments: { id: string }[] } | undefined;
            return acc + (p?.comments?.length ?? 0);
        }, 0);
        const countObj = cached.pages[0]?.find((item) => 'commentsCount' in item) as { commentsCount: number } | undefined;
        const totalCount = countObj?.commentsCount ?? undefined;
    ...
    const {
        ...
        getNextPageParam: (lastPage, allPages) => {
            ...
            const countObj = lastPage.find((item) => 'commentsCount' in item) as { commentsCount: number } | undefined;
            if (!commentsPage) return undefined;

            // 지금까지 로드된 총 댓글 개수
            const loadedCount = (allPages ?? []).reduce((acc: number, page: any) => {
                const p = page.find((it: any) => 'comments' in it) as { comments: { id: string }[] } | undefined;
                return acc + (p?.comments?.length ?? 0);
            }, 0);

            // 총 개수만큼 모두 로드했다면 다음 페이지 없음
            if (typeof countObj?.commentsCount === 'number' && loadedCount >= countObj.commentsCount) return undefined;
    ...
    const { mutateAsync: updateComment, isPending: isUpdatingComment } = useMutation({
        ...
        onSuccess: (updated) => {
            ...
            const updatedComment = (updated && typeof updated === 'object' && 'updatedComment' in updated)
                ? (updated as any).updatedComment
                : updated;

            // 최종 서버 데이터로 해당 댓글만 치환
            if (updatedComment?.id) {
                replaceCommentById(queryClient, blogId, updatedComment.id, updatedComment as any);
            }

            // 일부 페이지만 로드된 경우에만 서버 원본 재검증
            if (!isAllCommentsLoaded()) {
                queryClient.invalidateQueries({ queryKey: blogsCommentsKey(blogId), exact: true });
            }

            // 소켓 브로드캐스트(댓글 수정)
            try {
                socket?.emit(SOCKET_EVENTS.BLOG_COMMENT_UPDATED, { blogId, comment: updatedComment });
            } catch {}
        },
        onError: (err: any, _vars, ctx) => {
            if (ctx?.prev) queryClient.setQueryData(blogsCommentsKey(blogId), ctx.prev as any);
                    ...
                    <div className='mt-2'>
                        {data?.pages?.map((page, pageIndex) => (
                            <ul key={`page-${pageIndex}`}>
                                {page?.find(item => 'comments' in item)?.comments?.map((comment: CommentType) => (
                                        <li 
                                            key={comment.id}
                                        ...
                                        >
                                            ...
                                            <div className='flex-1'>
                                                <div className='inline-flex items-end flex-wrap'>
                                                    ...
                                                    {(user?.role === 'admin' || user?.email === comment.author?.email) && (
                                                ...
                                                {editingId === comment.id ? (
                                                    isUpdatingComment ? (
                                                        ...
                                                        <FormComment
                                                            ...
                                                            submitLabel='저장'
                                                            autoFocus
                                                            onSubmit={(text) => updateComment({ blogId, commentId: comment.id, text })}
