...
// ✅ 정적 데이터 외부 추출
const INTERNATIONAL_DATA = drinksData.filter((drink) => drink.type === "international");
const POSITION_GROUP = ["3vw", "6vw", "9vw"];
    ...
    <div className="product-layout" ref={scrollRef}>
      ...
      <div
        className="
            ...
            "
      >
        {INTERNATIONAL_DATA.map((product, index) => (
          <motion.div
            key={index}
            initial={{ y: POSITION_GROUP[index] }}
            whileInView={{ y: 0 }}
            viewport={INTERNATIONAL_MOTION_VIEWPORT}
            transition={INTERNATIONAL_MOTION_TRANSITION}
          >
            <Link
              href={product.link}
              title={product.name}
