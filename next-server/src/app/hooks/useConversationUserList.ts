...
export const useConversationUserList = create<ConversationUserListProps>()(
  (set) => ({
    ...
    add: (data) =>
      set((state) => {
        const exists = state.conversationUsers.some((_data) =>
            _data.conversationId === data.conversationId &&
            arraysEqualUnordered(_data.userIds, data.userIds),
        );
    ...
    remove: (id) =>
      set((state) => ({
        conversationUsers: state.conversationUsers.filter((_data) => 
          _data.conversationId !== id,
        ),
      })),
    set: (data) =>
      set((state) => {
        // 대상 대화방이 존재하는지 먼저 확인
        const existingIndex = state.conversationUsers.findIndex((conv) => 
        ...
        newConversationUsers[existingIndex] = {
          ...existing,
